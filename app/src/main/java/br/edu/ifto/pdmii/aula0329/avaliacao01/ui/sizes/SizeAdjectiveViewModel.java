...
public class SizeAdjectiveViewModel extends AndroidViewModel {

    private MutableLiveData<List<PokemonWithAbilities>> mPokemonsWithAbilities;

    public SizeAdjectiveViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    private void loadPokemonsWithAbilities() {
        ...
        executorService.execute(() -> {
            ...
            List<PokemonWithAbilities> pokemonWithAbilities = new ArrayList<>();

            pokemonWithAbilities.add(pokemonDao.findByMaxHeight());
            pokemonWithAbilities.add(pokemonDao.findByMaxWeight());
            pokemonWithAbilities.add(pokemonDao.findByMinHeight());
            pokemonWithAbilities.add(pokemonDao.findByMinWeight());

            mPokemonsWithAbilities.postValue(pokemonWithAbilities);
        });
    }
}
